...
`VMRAY_FETCH_ANALYSIS_ARCHIVES` defaults to `false`. Full archives are very large on this server and are not retained automatically; detail, VTI, and sample raw payloads remain preserved. Archive acquisition requires an explicit deliberate repair/reprocessing workflow.
