...
def ingest(detail_payload, vti_payload, sample_payload, submission_payload):
    ...
    with connection() as conn, conn.cursor() as cur:
        ...
        cur.execute("INSERT INTO analysis_runs(sample_id,vmray_analysis_id,vmray_sample_id,vmray_submission_id,vmray_job_id,analysis_type,requested_duration_seconds,duration_bucket,created_at,started_at,completed_at,vmray_version,analysis_configuration,target_environment,status,failure_state,verdict,original_verdict,verdict_score,verdict_reason,is_failed,vti_behavioural_high,vti_nonbehavioural_high,vti_config_extraction_high,vti_unknown_category_high,vti_static_detector_high,vti_total,submission_created,submission_interface_name,round_id,parser_version) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,1,%s) ON CONFLICT(vmray_analysis_id) DO UPDATE SET ingested_at=now(),verdict=EXCLUDED.verdict,status=EXCLUDED.status,failure_state=EXCLUDED.failure_state,completed_at=EXCLUDED.completed_at,is_failed=EXCLUDED.is_failed,vti_behavioural_high=EXCLUDED.vti_behavioural_high,vti_nonbehavioural_high=EXCLUDED.vti_nonbehavioural_high,vti_config_extraction_high=EXCLUDED.vti_config_extraction_high,vti_unknown_category_high=EXCLUDED.vti_unknown_category_high,vti_static_detector_high=EXCLUDED.vti_static_detector_high,vti_total=EXCLUDED.vti_total RETURNING id",(sample_pk,analysis_id,a.get("analysis_sample_id"),submission,a.get("analysis_job_id"),analysis_type,requested,requested if requested in (60,120,180) else None,created,started,created,a.get("analysis_analyzer_version") or a.get("analysis_static_engine_version") or a.get("analysis_dynamic_engine_version"),json.dumps(config),a.get("analysis_platform"),a.get("analysis_result_str"),None if not is_failed else a.get("analysis_result_str"),verdict,a.get("analysis_verdict"),a.get("analysis_vti_score"),a.get("analysis_verdict_reason_description"),is_failed,vti_counts.behavioural_high,vti_counts.nonbehavioural_high,vti_counts.config_extraction_high,vti_counts.unknown_category_high,vti_counts.static_detector_high,vti_counts.total,submission_created,interface_name,PARSER_VERSION)); run=cur.fetchone()["id"]
        for v in vtis:
...
async def collect_once():
    ...
    with connection() as lock_conn:
        ...
        totals={"discovered":0,"new":0,"skipped_existing":0,"ingested":0,"updated":0,"failed":0}
        try:
            with connection() as conn,conn.cursor() as cur:
            ...
            with connection() as conn,conn.cursor() as cur:
                ...
                cur.execute("UPDATE ingestion_batches SET completed_at=now(),status=%s,discovered=%s,ingested=%s,failed=%s,new_count=%s,skipped_existing=%s,updated=%s,duration_seconds=%s WHERE id=%s",("partial" if totals["failed"] else "success",totals["discovered"],totals["ingested"],totals["failed"],totals["new"],totals["skipped_existing"],totals["updated"],duration,batch))
                cur.execute("UPDATE collector_status SET state=%s,last_success_at=now(),connectivity_ok=true,lag_seconds=%s,error_count=error_count+%s,last_cycle_duration_seconds=%s,last_discovered=%s,last_new=%s,last_skipped_existing=%s,last_updated=%s,last_failed=%s,message=%s WHERE singleton",("degraded" if totals["failed"] else "healthy",int((now-newest).total_seconds()) if newest else None,totals["failed"],duration,totals["discovered"],totals["new"],totals["skipped_existing"],totals["updated"],totals["failed"],f"New {totals['ingested']}; skipped {totals['skipped_existing']}"));conn.commit()
