...
def overview(request:Request):
    ...
    with connection() as conn,conn.cursor() as cur:
        ...
        cur.execute(f"SELECT completed_at::date day,count(*) analyses,count(DISTINCT sample_id) samples,count(*) FILTER(WHERE analysis_type='static') static,count(*) FILTER(WHERE analysis_type='dynamic') dynamic FROM analysis_runs r WHERE {where} AND completed_at>=%s GROUP BY 1 ORDER BY 1",args+(start,)); daily=cur.fetchall()
