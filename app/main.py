...
def overview(request:Request):
    ...
    with connection() as conn,conn.cursor() as cur:
        ...
        if metric=="runs":
            cur.execute(f"SELECT completed_at::date day,count(*) FILTER(WHERE analysis_type='static' AND static_repetition IN(1,2,3)) static,count(*) FILTER(WHERE analysis_type='dynamic' AND duration_bucket=60) dynamic_60,count(*) FILTER(WHERE analysis_type='dynamic' AND duration_bucket=120) dynamic_120,count(*) FILTER(WHERE analysis_type='dynamic' AND duration_bucket=180) dynamic_180,count(*) FILTER(WHERE NOT ((analysis_type='static' AND static_repetition IN(1,2,3)) OR (analysis_type='dynamic' AND duration_bucket IN(60,120,180)))) other FROM analysis_runs r WHERE {where} AND completed_at>=%s GROUP BY 1 ORDER BY 1",args+(start,)); raw_daily=cur.fetchall(); keys=("static","dynamic_60","dynamic_120","dynamic_180","other")
        elif metric=="samples":
            cur.execute(f"SELECT first_seen::date day,count(*) total FROM samples s WHERE {sample_where} AND first_seen>=%s GROUP BY 1 ORDER BY 1",sample_args+(start,)); raw_daily=cur.fetchall(); keys=("total",)
        else:
            cur.execute(f"SELECT created_at::date day,count(*) total FROM sample_analysis_groups g WHERE {group_where} AND created_at>=%s GROUP BY 1 ORDER BY 1",group_args+(start,)); raw_daily=cur.fetchall(); keys=("total",)
